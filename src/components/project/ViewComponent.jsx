...
const ViewComponent = ({ data, tab, journey, closeModal }) => {
      ...
      return (
        <div>
          ...
          <div className="flex flex-col items-start mt-5 p-2">
            ...
            <p className="text-md my-1 text-left leading-8 ">
              {data["BMC_summary"]}
            </p>
          </div>
        </div>
      ...
      return (
        <div className="flex flex-col items-start justify-start space-y-4 ">
          ...
          <p className="text-sm text-center font-semibold text-red-500">
            Disclaimer: Research Tab is in private beta, it may not give
            accurate results all the time and is prone to hallucinations. We are
            working on it
          </p>
          <div className="flex flex-col items-start justify-start space-y-2 ">
            <p className="text-gray-700 text-lg">
              Research and Knowledge Bank provides the best resources that we
              could curate for you.
              {/* <br /> */}&nbsp; They include data banks, competitior
              showcasing or just more information that we believe should be
              useful for you as a founder.
            </p>
          </div>
          {data?.competitors && (
            <div className="flex flex-col items-start justify-start space-y-2 ">
              ...
              <table className="table-auto ">
                <thead>
                  <tr>
                    <th className="px-4 py-2 border border-gray-500">Sr. No</th>
                ...
                {sortedData.map((item, index) => (
                  <tbody key={index}>
                    <tr>
                      ...
                      <td className=" px-4 py-2 border border-gray-500">
                        <Markdown className="prose w-full text-lg text-black">
                          {item?.description}
                        </Markdown>
                      </td>
                      <td className=" px-4 py-2 border border-gray-500">
            ...
            {data &&
              Object.entries(data).map((item, index) => (
                <tbody key={index}>
                  <tr>
                    <td
                      className="px-4 py-2 text-brand border-b-2 font-medium w-[200px]"
                      style={{ verticalAlign: "top" }}
                    >
                      {item[0]}
                    </td>
                    <td className="px-4 py-2 border-b-2 border-l-2 font-medium">
                      {item[0].startsWith("Brand Colour") ? (
                        <div className="flex items-center">
                          <div
                            className="w-5 h-5 mr-2 rounded-sm"
                            style={{
                              backgroundColor: extractHexColorsFromString(
                                item[1]
                              ),
            ...
            {data["ps_list"]?.map((item, index) => (
              <tbody key={index}>
                <tr>
                  ...
                  <td className=" px-4 py-2 text-brand border-b-2 border-l-2 font-medium">
                    {item?.Solution}
                  </td>
                </tr>
              </tbody>
            ...
            {data?.map((item, index) => {
              return (
                ...
                <tbody key={index}>
                  <tr>
                    <td
                      className="px-4 py-2 font-semibold text-brand whitespace-nowrap"
                      style={{ verticalAlign: "top" }}
                    >
                      {item.key}
                    </td>
                ...
                >
                  <ul className="list-none px-4">
                    {item?.Tasks.map((task, index) => {
                      const priority =
                        item["Priorities"] && item["Priorities"][index];
                      return (
